...
def compute_mean_embedding(node_embeddings, label_dict):
    mean_embeddings = {}
    for label, node_ids in label_dict.items():
        embeddings = node_embeddings[node_ids]  
        mean_embeddings[label] = embeddings.mean(dim=0) 
    return mean_embeddings

def compute_euclidean_distance(embedding, mean_embedding):
    return torch.norm(embedding - mean_embedding)


def get_closest_nodes(node_embeddings, label_dict):
    mean_embeddings = compute_mean_embedding(node_embeddings, label_dict)
    closest_nodes = {}
    sample_list=[]
    sample_embedding = torch.empty((0, node_embeddings.shape[1]), device=node_embeddings.device)
    ...
    for label, node_ids in label_dict.items():
        ...
        for node_id in node_ids:
            ...
            distance = compute_euclidean_distance(embedding, mean_embedding) 
            distances.append((node_id, distance.item()))

       
        distances.sort(key=lambda x: x[1])
        closest_nodes[label] = [node_id for node_id, _ in distances[:30]]
        sample_list.extend(closest_nodes[label])
    sample_list.sort()
    
    for node_id in sample_list:
        sample_embedding = torch.cat((sample_embedding, node_embeddings[node_id].unsqueeze(0)), dim=0)

    return sample_list, sample_embedding

def combine_all_params(folder_path, n):
 
    param_files = [f for f in os.listdir(folder_path) if f.endswith('dis.pkl')]
    

    param_files = [f for f in param_files if f.split('_')[0].isdigit() and int(f.split('_')[0]) < n]
    param_files = sorted(param_files, key=lambda x: int(x.split('_')[0]))  

    if not param_files:
        raise FileNotFoundError(f"not found 0 to {n}'.pkl' files!")

    combined_params = None
    file_count = 0
    encoder_c_weighted_sum = None  
    S = sum(range(1, n + 1))  

    for idx, file_name in enumerate(param_files):
        file_path = os.path.join(folder_path, file_name)
        print(f"deal files: {file_name}")


        with open(file_path, 'rb') as f:
            current_params = torch.load(f)
        if n==1:
        ...
        if combined_params is None:
            ...
            encoder_c_weighted_sum = OrderedDict()

            for key, value in current_params.items():
                combined_params[key] = value.clone()  
                if "encoder_c" in key:
                    encoder_c_weighted_sum[key] = (n - idx) / S * value.clone() 
        else:
            for key in combined_params.keys():
                if "encoder_c" in key:
                    if encoder_c_weighted_sum[key].size() == current_params[key].size():
                        encoder_c_weighted_sum[key] += (n - idx) / S * current_params[key]
                    else:
                        raise ValueError(f"Size mismatch for key {key}: {encoder_c_weighted_sum[key].size()} vs {current_params[key].size()}")
                elif "encoder_s" in key or "decoder" in key or "discriminator" in key:
                    combined_params[key] += current_params[key]

        file_count += 1

    for key in combined_params.keys():
        if "encoder_c" in key:
            combined_params[key] = encoder_c_weighted_sum[key]
        elif "encoder_s" in key or "decoder" in key or "discriminator" in key:
            combined_params[key] = (combined_params[key] / file_count).type(combined_params[key].dtype) 
    print(f"combine {file_count} files!")
